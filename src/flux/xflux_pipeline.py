...
class XFluxPipeline:
    ...
    def set_controlnet(
        ...
        self.model.to(self.device)
        self.controlnet = load_controlnet(self.model_type, self.device).to(
            torch.bfloat16
        )
    ...
    def __call__(
        ...
        if not (image_prompt is None and neg_image_prompt is None):
            assert (
                self.ip_loaded
            ), "You must setup IP-Adapter to add image prompt as input"
        ...
        if self.controlnet_loaded:
            ...
            controlnet_image = torch.from_numpy(
                (np.array(controlnet_image) / 127.5) - 1
            )
            controlnet_image = (
    ...
    def gradio_generate(
        ...
        if controlnet_image is not None:
            ...
            if (
                self.controlnet_loaded and control_type != self.control_type

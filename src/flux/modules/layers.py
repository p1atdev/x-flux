...
class DoubleStreamBlockLoraProcessor(nn.Module):
    ...
    def forward(self, attn, img, txt, vec, pe, **attention_kwargs):
        ...
        img = (
            img
            + img_mod1.gate * attn.img_attn.proj(img_attn)
            + img_mod1.gate * self.proj_lora1(img_attn) * self.lora_weight
        )
        ...
        txt = (
            txt
            + txt_mod1.gate * attn.txt_attn.proj(txt_attn)
            + txt_mod1.gate * self.proj_lora2(txt_attn) * self.lora_weight
        )
...
class IPDoubleStreamBlockProcessor(nn.Module):
    ...
    def __init__(self, context_dim, hidden_dim):
        ...
        self.ip_adapter_double_stream_k_proj = nn.Linear(
            context_dim, hidden_dim, bias=True
        )
        self.ip_adapter_double_stream_v_proj = nn.Linear(
            context_dim, hidden_dim, bias=True
        )
    ...
    def __call__(
        self, attn, img, txt, vec, pe, image_proj, ip_scale=1.0, **attention_kwargs
    ):
        # Prepare image for attention
        img_mod1, img_mod2 = attn.img_mod(vec)
        txt_mod1, txt_mod2 = attn.txt_mod(vec)
        ...
        ip_key = rearrange(
            ip_key, "B L (H D) -> B H L D", H=attn.num_heads, D=attn.head_dim
        )
        ip_value = rearrange(
            ip_value, "B L (H D) -> B H L D", H=attn.num_heads, D=attn.head_dim
        )
        ...
        ip_attention = rearrange(
            ip_attention, "B H L D -> B L (H D)", H=attn.num_heads, D=attn.head_dim
        )
...
class IPSingleStreamBlockProcessor(nn.Module):
    ...
    def __call__(
        ...
        q, k, v = rearrange(
            qkv, "B L (K H D) -> K B H L D", K=3, H=attn.num_heads, D=attn.head_dim
        )
        q, k = attn.norm(q, k, v)

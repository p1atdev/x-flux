...
def main():
    ...
    controlnet = load_controlnet(
        name=args.model_name, device=accelerator.device, transformer=dit
    )
    ...
    print(
        sum([p.numel() for p in controlnet.parameters() if p.requires_grad]) / 1000000,
        "parameters",
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                x_t = (
                    1
                    - t.unsqueeze(1).unsqueeze(2).repeat(1, x_1.shape[1], x_1.shape[2])
                ) * x_1 + t.unsqueeze(1).unsqueeze(2).repeat(
                    1, x_1.shape[1], x_1.shape[2]
                ) * x_0
                bsz = x_1.shape[0]

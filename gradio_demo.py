    ...
    with gr.Blocks() as demo:
        ...
        with gr.Tab("Inference"):
            with gr.Row():
                with gr.Column():
                    prompt = gr.Textbox(
                        label="Prompt", value="handsome woman in the city"
                    )
                    ...
                    with gr.Accordion("Generation Options", open=False):
                        ...
                        neg_prompt = gr.Textbox(
                            label="Negative Prompt", value="bad photo"
                        )
                        with gr.Row():
                            num_steps = gr.Slider(
                            ...
                            timestep_to_start_cfg = gr.Slider(
                                1, 50, 1, step=1, label="timestep_to_start_cfg"
                            )
                        with gr.Row():
                            guidance = gr.Slider(
                                1.0,
                    ...
                    with gr.Accordion("ControlNet Options", open=False):
                        control_type = gr.Dropdown(
                            ["canny", "hed", "depth"], label="Control type"
                        )
                    ...
                    with gr.Accordion("IP Adapter Options", open=False):
                        image_prompt = gr.Image(
                            label="image_prompt", visible=True, interactive=True
                        )
                        ip_scale = gr.Slider(0.0, 1.0, 1.0, step=0.1, label="ip_scale")
                        neg_image_prompt = gr.Image(
                            label="neg_image_prompt", visible=True, interactive=True
                        )
                        neg_ip_scale = gr.Slider(
                            0.0, 1.0, 1.0, step=0.1, label="neg_ip_scale"
                        )
                        ip_local_path = gr.Dropdown(
                            checkpoints,
                            label="IP Adapter Checkpoint",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=".",
        help="Folder with checkpoints in safetensors format",
    )
